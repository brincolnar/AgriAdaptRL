...
if __name__ == "__main__":
    NUM_TRAIN_DATASETS = 3
    dataset_files = [f'dataset_{i}.csv' for i in range(1, NUM_TRAIN_DATASETS + 1)]
    verbose = True 
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # Train
    for i in range(0, NUM_TRAIN_DATASETS):
        # Define test dataset for the current fold
        test_file = dataset_files[i]
        train_files = [f for f in dataset_files if f != test_file]

        print(f"Fold {i}")
        print(f"Train files: {train_files}, test file: {test_file}")

        # Reinitialize models and optimizer for new fold
        ...
        for train_file in train_files:
            ...
            train_env = NetworkSelectionEnvImages(dataframe=train_df, image_folder='./data/ordered_train_test/all/images/', performance_factor=PERFORMANCE_FACTOR, device=device, verbose=False)
            ...
            # Print the results
            print(f"File {train_file} completed.")
            ...
            plt.tight_layout()
            plt.savefig(f"./results_{train_file}.png", bbox_inches='tight')
            ...
            plt.ylabel('Loss')
            plt.legend()
            plt.savefig(f"./loss_{train_file}.png", bbox_inches='tight')
            print("----------------------------------------------------------")
        ...
        for t in tqdm(range(len(test_env.dataframe)), desc=f'Testing and training...'):
            # Select action
            if random.random() > EPSILON:
                with torch.no_grad():
                    action = model(state).argmax().item()
            ...
            ious.append(test_env.dataframe.iloc[test_env.current_idx][action_to_network_mapping[action]])
            weights.append(network_to_weight_mapping[action])
        ...
        plt.tight_layout()
        plt.savefig(f"./results_{test_file}.png", bbox_inches='tight')
        ...
        plt.ylabel('Loss')
        plt.legend()
        plt.savefig(f"./loss_{test_file}.png", bbox_inches='tight')
        print("----------------------------------------------------------")