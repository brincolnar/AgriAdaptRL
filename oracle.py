...
class OracleComputation:
    ...
    def infer(self, file_name, fixed=-1):
        ...
        return results, filename, best_width, best_metric
...
if __name__ == "__main__":
    ...
    all_images_path = "./data/geok/test/images/"

    iou = 0

    for path in os.listdir(all_images_path):
        results, filename, best_width, best_metric = si.infer(path)
        iou += best_metric
        print(f"Filename: {filename}, Best Width: {best_width} Best_metric: {best_metric}")
