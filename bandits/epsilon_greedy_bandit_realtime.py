...
class ContextBandit:
    def __init__(self, n_actions, n_features, image_dir, label_dir, model_paths, performance_factor, epochs, learning_rate=0.01, epsilon=0.10):
        ...
        all_features = self.features_file[self.feature_columns].values
        ...
        self.image_list = sorted(os.listdir(image_dir), key=self.get_frame_number)


    def get_frame_number(self, filename):
    	match = re.search(r'frame_(\d+)\.jpeg', filename)
    	if match:
    ...
    # For realtime feature computation:
    # def load_image_features(self, image_path):
    #     features = self.compute_image_features(image_path)
    #     return features

    def load_image_features(self):
        if self.current_image_index < 457:
            row = self.features_file.iloc[self.current_image_index]
    ...
    def calculate_reward(self, selected_network, image_name):
        results, _, _, _, _ = self.segments[selected_network].infer(image_name)
    ...
    def train(self, epochs):
        ...
        csv_path = os.path.join('./results/epsilon_features/', csv_filename)

        # This is for 457 images
        battery = 100
        ...
        with open(csv_path, 'w', newline='') as csvfile:
            ...
            for epoch in range(epochs):
                for image_name in tqdm(self.image_list, desc="Training Progress"):
                    ...
                    weight = network_to_weight[selected_network_name]
                    total_weight += weight
                    ...
                    filename_iou_list.append({'Filename': image_name, 'Epsilon': iou, 'EpsilonWeight': network_to_weight[selected_network_name] })     
